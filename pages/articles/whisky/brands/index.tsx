...
export default function BrandsIndex({ articles }: BrandsIndexProps) {
  return (
    <div className="min-h-screen bg-gray-50">
      <Header 
        title="銘柄紹介一覧 - ウイスキー | Market Supporter AI"
        description="ウイスキーの銘柄紹介記事の過去記事一覧です。"
      />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="bg-white rounded-2xl shadow-md p-8">
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900 mb-2">銘柄紹介一覧</h1>
            <p className="text-gray-600">ウイスキーの銘柄紹介記事の過去記事をすべて表示しています。</p>
          </div>
          ...
          {articles.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">銘柄紹介記事を準備中です。近日公開予定です。</p>
            </div>
          ) : (
...
export const getStaticProps: GetStaticProps = async () => {
  const articlesDirectory = path.join(process.cwd(), 'articles/whisky/brands');
  const articles: Article[] = [];
  
  if (fs.existsSync(articlesDirectory)) {
    const files = fs.readdirSync(articlesDirectory).filter(f => f.endsWith('.md') || f.endsWith('.mdx'));
    files.forEach(file => {
      ...
      articles.push({
        ...
        date: frontMatter.date || '2025-11-12',
      });
    });
  }
