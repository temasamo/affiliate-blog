...
export default function Group5Index({ articles }: { articles: Article[] }) {
  return (
        ...
        <div className="bg-white rounded-2xl shadow-md p-8">
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900 mb-2">
              枕診断シリーズ｜グループ5（朝スッキリ編）
            </h1>
            <p className="text-gray-600">
              朝起きてもだるい、疲れが取れない…そんな悩みを解決する枕選びのポイントを紹介します。
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {articles.map((a) => (
              <Link
                key={a.slug}
                href={`/articles/sleep-health/pillow/group5/${a.slug}`}
                className="group"
              >
...
export async function getStaticProps() {
  const dir = path.join(process.cwd(), "articles", "sleep-health", "pillow", "group5");
    ...
    files
      ...
      .forEach((file) => {
        ...
        articles.push({
          ...
          date: frontMatter.date || "2025.10.11",
        });
      });
  }
