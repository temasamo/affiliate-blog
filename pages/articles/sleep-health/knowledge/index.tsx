// pages/articles/sleep-health/knowledge/index.tsx
...
export default function KnowledgeIndex({ articles }: KnowledgeIndexProps) {
  return (
    <div className="min-h-screen bg-gray-50">
      <Header 
        title="睡眠知識一覧 - 睡眠・健康 | Market Supporter AI"
        description="睡眠・健康に関する知識記事の過去記事一覧です。"
      />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="bg-white rounded-2xl shadow-md p-8">
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900 mb-2">睡眠知識一覧</h1>
            <p className="text-gray-600">睡眠・健康に関する知識記事の過去記事をすべて表示しています。</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {articles.map((article) => (
              <Link key={article.slug} href={`/articles/sleep-health/knowledge/${article.slug}`} className="group">
...
export const getStaticProps: GetStaticProps<KnowledgeIndexProps> = async () => {
  const articlesDirectory = path.join(process.cwd(), 'articles', 'sleep-health', 'knowledge');
