...
export default function HealthIndex({ articles }: HealthIndexProps) {
  return (
        ...
        <div className="bg-white rounded-2xl shadow-md p-8">
          <div className="text-center mb-8">
            <div className="relative h-32 mb-6 rounded-xl overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-rose-400/80 to-pink-600/80"></div>
            ...
            <p className="text-gray-600 text-lg leading-relaxed max-w-3xl mx-auto">
              冬の乾燥やコンディションに寄り添う、やさしいケアの視点をまとめています。
              肌と眠りの関係を丁寧に整える記事をお届けします。
            </p>
          </div>
          ...
          {articles.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">健康（美容）記事を準備中です。近日公開予定です。</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {articles.map((article) => (
                <Link
                  key={article.slug}
                  href={`/articles/sleep-health/health/${article.slug}`}
                  className="group block bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow border border-gray-100"
                >
                  <div className="p-6">
                    <div className="text-3xl mb-3">🧴</div>
                    <h3 className="text-lg font-semibold text-gray-900 group-hover:text-rose-600 transition-colors mb-2">
                      {article.title}
                    </h3>
...
export const getStaticProps: GetStaticProps<HealthIndexProps> = async () => {
  const articlesDirectory = path.join(process.cwd(), "articles/sleep-health/health");
