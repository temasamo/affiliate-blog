// pages/articles/popularproducts-overseas/recomend/index.tsx
...
export default function RecomendIndex({ articles }: RecomendIndexProps) {
  return (
    <div className="min-h-screen bg-gray-50">
      <Header 
        title="おすすめ商品一覧 - 海外トレンド | Market Supporter AI"
        description="海外トレンドに関するおすすめ商品の過去記事一覧です。"
      />
      
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="bg-white rounded-2xl shadow-md p-8">
          <div className="mb-8">
            <Link href="/overseas-trend" className="inline-flex items-center text-blue-600 hover:text-blue-800 mb-4">
            ...
            <p className="text-gray-600">海外トレンドに関するおすすめ商品の過去記事をすべて表示しています。</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {articles.map((article) => (
              <Link key={article.slug} href={`/articles/popularproducts-overseas/recomend/${article.slug}`} className="group">
...
export const getStaticProps: GetStaticProps<RecomendIndexProps> = async () => {
  const articlesDirectory = path.join(process.cwd(), 'articles', 'popularproducts-overseas', 'recomend');
