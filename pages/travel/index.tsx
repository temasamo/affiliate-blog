...
export default function TravelIndex({ posts }: { posts: any[] }) {
  return (
    <main className="mx-auto max-w-5xl p-6 sm:p-10">
      <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-6">旅行カテゴリ</h1>
      ...
      <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {posts.map((p) => (
          <li key={p.slug}>
            <Link
              href={`/travel/${p.slug}`}
              className="block rounded-2xl border border-gray-200 bg-white p-4 hover:bg-gray-50 hover:shadow-lg transition"
            >
