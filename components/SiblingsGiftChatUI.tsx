...
import { siblingsQuestionFlow, generateSiblingsSuggestions, SiblingsGiftItem } from '@/lib/gift_items_siblings_v2';
...
interface SiblingsGiftChatUIProps {
  version?: string;
}

const SiblingsGiftChatUI: React.FC<SiblingsGiftChatUIProps> = ({ version = "v2" }) => {
  const [chat, setChat] = useState<ChatMessage[]>([]);
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<string, string | string[]>>({});
  const [suggestions, setSuggestions] = useState<SiblingsGiftItem[]>([]);
  ...
  useEffect(() => {
    ...
    const greetingMessage: ChatMessage = {
      id: 'greeting',
      from: 'bot',
      text: 'こんにちは！兄弟姉妹への素敵なギフトを一緒に選びましょう。\n\n兄弟姉妹の趣味や好み、一緒に楽しめるポイントなど、色々お伺いさせていただきますね。',
      timestamp: new Date()
    };
  ...
  const addQuestion = (questionIndex: number) => {
    if (questionIndex >= siblingsQuestionFlow.length) {
      // 全ての質問が完了したら提案を生成
      generateSuggestions();
      return;
    }

    const question = siblingsQuestionFlow[questionIndex];
  ...
  const handleFreeTextSubmit = () => {
    if (freeTextInput.trim()) {
      const currentQuestion = siblingsQuestionFlow[currentQuestionIndex];
  ...
  const generateSuggestions = async () => {
    try {
      const generatedSuggestions = generateSiblingsSuggestions(answers);
      ...
      const suggestionMessage: ChatMessage = {
        id: 'suggestions',
        from: 'bot',
        text: `兄弟姉妹にぴったりのギフトを${generatedSuggestions.length}つ提案させていただきました！\n\n趣味・実用・体験を重視した、兄弟姉妹が喜ぶ特別なギフトです。`,
        timestamp: new Date()
      };
  ...
  const getCurrentQuestion = () => {
    if (currentQuestionIndex < siblingsQuestionFlow.length) {
      return siblingsQuestionFlow[currentQuestionIndex];
    }
    return null;
  };
            ...
            {suggestions.map((item, index) => (
              <div key={item.id} className="bg-white rounded-xl p-4 shadow-md">
                <div className="text-center mb-3">
                  ...
                  <p className="text-sm text-purple-600 font-medium">{item.priceRange}</p>
                </div>
